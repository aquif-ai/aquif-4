# coding=utf-8
# Copyright 2025 aquif AI and The HuggingFace Inc. team. All rights reserved.
#
...
from .configuration_aquif_4_linear import Aquif4LinearConfig
...
_CONFIG_FOR_DOC = "Aquif4LinearConfig"
...
@dataclass
class Aquif4CausalLMOutputWithPast(ModelOutput):
    """
...
class Aquif4ModelOutputWithPast(MoeModelOutputWithPast):
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    warnings.warn(
        "Calling `transformers.models.Aquif4Linear.modeling_aquif_4_linear._prepare_4d_attention_mask` is deprecated and will be removed in v4.37. Use `transformers.modeling_attn_mask_utils._prepare_4d_attention_mask"
    )
...
def _make_causal_mask(
    ...
    warnings.warn(
        "Calling `transformers.models.Aquif4Linear.modeling_aquif_4_linear._make_causal_mask` is deprecated and will be removed in v4.37. Use `transformers.models.Aquif4Linear.modeling_aquif_4_linear.AttentionMaskConverter._make_causal_mask"
    )
...
class Aquif4LinearRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Aquif4LinearRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class Aquif4LinearGroupRMSNorm(nn.Module):
...
ALL_LAYERNORM_LAYERS.append(Aquif4LinearRMSNorm)


class Aquif4RotaryEmbedding(nn.Module):
    def __init__(self, config: Aquif4LinearConfig, device=None):
        super().__init__()
...
class Aquif4LinearMLP(nn.Module):
    def __init__(self, config: Aquif4LinearConfig, intermediate_size: int):
        super().__init__()
        self.config = config
...
class Aquif4LinearGate(nn.Module):
...
class Aquif4LinearSparseMoeBlock(nn.Module):
    """
    A mixed expert module containing shared experts.
    """

    def __init__(self, config: Aquif4LinearConfig):
        super().__init__()
        self.config = config
        self.num_experts_per_tok = config.num_experts_per_tok
        self._setup_experts()
        self.gate = Aquif4LinearGate(config)
        if config.num_shared_experts is not None:
            self.shared_experts = Aquif4LinearMLP(
    ...
    def _setup_experts(self):
        self.experts = nn.ModuleList(
            [
                Aquif4LinearMLP(config=self.config, intermediate_size=self.config.moe_intermediate_size)
...
class Aquif4LinearAttention(nn.Module):
    

    def __init__(self, config: Aquif4LinearConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        if self.config.use_qk_norm:
            self.query_layernorm = Aquif4LinearRMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.key_layernorm = Aquif4LinearRMSNorm(self.head_dim, eps=config.rms_norm_eps)
...
class Aquif4LinearFlashAttention2(Aquif4LinearAttention):
    """
    Aquif4Linear flash attention module. This module inherits from `Aquif4LinearAttention` as the weights of the module stays
...
class Aquif4LinearSdpaAttention(Aquif4LinearAttention):
    """
    Aquif4Linear attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `Aquif4LinearAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Aquif4LinearModel is using Aquif4LinearSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
ATTENTION_CLASSES = {
    "eager": Aquif4LinearAttention,
    "flash_attention_2": Aquif4LinearFlashAttention2,
    "sdpa": Aquif4LinearSdpaAttention,
}


class Aquif4LinearAttention(nn.Module):
    """
    Aquif4LinearAttention implements a linear attention mechanism based on Lightning Attention-2
    ...
    def __init__(self, config: Aquif4LinearConfig, layer_idx: Optional[int] = None):
        ...
        self.rotary_emb = Aquif4RotaryEmbedding(config=config)

        self.dense = nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=config.use_bias)

        self.g_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        self.g_norm = Aquif4LinearGroupRMSNorm(self.num_heads * self.head_dim, group_norm_size=config.group_norm_size, eps=self.rms_norm_eps)
        slope = - Aquif4LinearAttention.build_slope_tensor(self.num_heads) * (1 - (self.layer_idx - 1) / (self.config.num_hidden_layers - 1) + 1e-5)
    ...
    def build_slope_tensor(n_attention_heads: int):
        ...
        Note:
            Code copied from: https://github.com/OpenNLPLab/lightning-attention/blob/d15c38529bbd5c2c82b44ddda3cac885825aa873/lightning_attn/utils/utils.py
        """    
        def get_slopes(n):
...
class Aquif4LinearMTPLayer(nn.Module):
    def __init__(self, config: Aquif4LinearConfig, layer_idx: int):
        super().__init__()
        self.layer_idx = layer_idx
        self.input_layernorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.enorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)

        self.eh_proj = nn.Linear(config.hidden_size * 2, config.hidden_size, bias=False)
        self.post_attention_layernorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.attention = ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
        self.mlp = Aquif4LinearSparseMoeBlock(config)

        self.hnorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.final_layernorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Aquif4LinearDecoderLayer(nn.Module):
    def __init__(self, config: Aquif4LinearConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        if self.attention_layer_type == "attention":
            ...
            self.attention = Aquif4LinearAttention(
                config=config,
                layer_idx=layer_idx
            )

        self.mlp = (
            Aquif4LinearSparseMoeBlock(config)
            if (config.num_experts is not None and layer_idx >= config.first_k_dense_replace)
            else Aquif4LinearMLP(config=config, intermediate_size=config.intermediate_size)
        )
        self.input_layernorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
AQUIF4LINEAR_START_DOCSTRING = r"""
...
@add_start_docstrings(
    "The bare Aquif4Linear Model outputting raw hidden-states without any specific head on top.",
    AQUIF4LINEAR_START_DOCSTRING,
)
class Aquif4LinearPreTrainedModel(PreTrainedModel):
    config_class = Aquif4LinearConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Aquif4LinearDecoderLayer"]
...
AQUIF4LINEAR_INPUTS_DOCSTRING = r"""
    Args:
...
class Aquif4LinearModel(Aquif4LinearPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Aquif4LinearDecoderLayer`]
    Args:
        config: Aquif4LinearConfig
    """
    ...
    def __init__(self, config: Aquif4LinearConfig):
        ...
        for layer_idx in range(config.num_hidden_layers + config.num_nextn_predict_layers):
            layer_cls = Aquif4LinearDecoderLayer if layer_idx < config.num_hidden_layers else Aquif4LinearMTPLayer
        ...
        self.norm = Aquif4LinearRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Aquif4RotaryEmbedding(config=config)
    ...
    @add_start_docstrings_to_model_forward(AQUIF4LINEAR_INPUTS_DOCSTRING)
    def forward(
        self,
        ...
        **kwargs,
    ) -> Union[Tuple, Aquif4ModelOutputWithPast]:
        ...
        return Aquif4ModelOutputWithPast(
...
class Aquif4LinearForCausalLM(Aquif4LinearPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: Aquif4LinearConfig):
        super().__init__(config)
        self.model = Aquif4LinearModel(config)
        self.vocab_size = config.vocab_size
    ...
    @add_start_docstrings_to_model_forward(AQUIF4LINEAR_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Aquif4CausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        return_dict: Optional[bool] = None,
        **kwargs,
    ) -> Union[Tuple, Aquif4CausalLMOutputWithPast]:
        r"""
        Args:
        ...
        >>> model = Aquif4LinearForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
        ...
        return Aquif4CausalLMOutputWithPast(
            loss=loss,
